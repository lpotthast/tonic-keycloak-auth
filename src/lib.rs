//! Protect tonic routes with a JWT emitted by Keycloak.
//!
//! # Usage
//!
...
//!
//! Let's set up a protected tonic route!
//!
...
//! ```rust
//! use std::sync::Arc;
//! use tonic_keycloak_auth::{Url, error::AuthError, instance::KeycloakConfig, instance::KeycloakAuthInstance, interceptor::KeycloakAuthInterceptor, decode::KeycloakToken, PassthroughMode, expect_role};
//!
...
//!         .layer(
//!              KeycloakAuthInterceptor::<String>::builder()
//!                  .instance(instance)
...
//!
//! // The `KeycloakAuthLayer` makes the parsed token data available using tonic's `Extension`'s,
...
//!     // tonic::serve(tcp_listener, router.into_make_service()).await.unwrap();
//! }
//! ```
//!
...
//!
//! This is the type representing a single role and can be replaced with any type implementing the `tonic_keycloak_auth::role::Role` trait.
//!
...
//!
//! use tonic::{http::StatusCode, response::{Response, IntoResponse}, Extension};
//! use tonic_keycloak_auth::{decode::KeycloakToken, expect_role};
//!
...
//! which allows for customized strategies on how to retrieve the token from an tonic request.
//!
...
//!
//! let layer = KeycloakAuthInterceptor::<String>::builder()
//!     .instance(instance)
//!     .passthrough_mode(PassthroughMode::Block)
//!     .expected_audiences(vec![String::from("account")])
//!     // ...
...
/// On successful authentication, the parsed token content is stored as an tonic extension as a `KeycloakToken`.
///
/// ```PassthroughMode::Pass```:  Forward to the response handler regardless of whether there was an authentication failure.
/// In this mode, the authentication status is stored as an tonic extension as a `KeycloakAuthStatus`.
