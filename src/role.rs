...
macro_rules! expect_roles {
    ($token: expr, $roles: expr) => {
        if let Err(err) = tonic_keycloak_auth::role::ExpectRoles::expect_roles($token, $roles) {
            return tonic::response::IntoResponse::into_response(err);
        }
    };
}
...
macro_rules! expect_role {
    ($token: expr, $role: expr) => {
        if let Err(err) = tonic_keycloak_auth::role::ExpectRoles::expect_roles($token, &[$role]) {
            return tonic::response::IntoResponse::into_response(err);
        }
    };
}
...
macro_rules! not_expect_roles {
    ($token: expr, $roles: expr) => {
        if let Err(err) = tonic_keycloak_auth::role::ExpectRoles::not_expect_roles($token, $roles) {
            return tonic::response::IntoResponse::into_response(err);
        }
    };
}
...
macro_rules! not_expect_role {
    ($token: expr, $role: expr) => {
        if let Err(err) = tonic_keycloak_auth::role::ExpectRoles::not_expect_roles($token, &[$role])
        {
            return tonic::response::IntoResponse::into_response(err);
        }
    };
