use crate::{
    error::DecodeSnafu,
    error::{AuthError, DecodeHeaderSnafu},
...
/// Token data parsed from the request and added as an `tonic::Extension` through our middleware.
///
...
/// `PassthroughMode::Pass` (potentially on a separate `tonic::Router`) and inject
