...
pub(crate) struct Action<I: ActionInput, O: ActionOutput> {
    ...
    /// Last starting time at which the operation was dispatched.
    #[educe(Debug(ignore))]
    ...
    /// `true` if there is an ongoing operation.
    pending: Arc<AtomicBool>,

    /// Notifies consumers waiting for a result.
    notify: Arc<Notify>,
...
mod test {
    ...
    async fn test_action_dispatch() {
        ...
        let notified = action.notified();
        
        let _dispatch_result = action.dispatch(String::from("result1")).await;

        assert_that(notified.await).is_equal_to(());
